...
public class E_MAIL_GONDERME extends JInternalFrame {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//
						 ...
						 boolean varmi = OBS_MAIN.pencere_bak("RAPOR YAZDIRMA");
						 if (varmi  ) 
		             		{
							 gonder();
		             		}
	...
	private void doldur_setting() 
	{
		try
		{
		ResultSet rs = oac.glb.mail_bak(GLOBAL.KULL_ADI);
		int count=0;
		rs.next();
		count = rs.getRow();
		if (count  == 0) return;
        ...
        cmbalici.addItem("");
         //
        ResultSet rss = oac.glb.alici_oku(GLOBAL.KULL_ADI);
	...
	//	  
		   ...
		   oac.glb.giden_rapor_yaz(new java.sql.Date(Calendar.getInstance().getTime().getTime())  ,txtkonu.getText(), rapor_dos_adi,  cmbalici.getSelectedItem().toString()  ,
