...
public class USER_EKLEME extends JInternalFrame {
	...
	public static void kayit() 
	{
	      ...
	      try
	      {
	        	...
	        	boolean varmi =  oac.glb.user_bak(txtkull.getText());
	            if ( varmi == true ) // ' USER VAR  
	            {
	                oac.glb.user_sil(txtkull.getText());
	            }
	           oac.glb.user_ekle_degis(txtkull.getText(),txtsif.getText(), txtsev.getText(), txtdb.getText(), txtmail.getText(),  chckbxl.isSelected()  ? true : false,  chckbxs.isSelected()  ? true :false);
	           grid_doldur();
			   kutu_temizle();
			   doldur_kutu(table_1,0);
	           txtkull.setEnabled(false);
	...
	public static void sil()
     {
   	  if ( txtkull.getText().equals("")) return;
   		   ...
   		   if(g != 0 ) { return;	}
    		   try
   		   {
   		      if (oac.glb.user_bak(txtkull.getText())) // USER VAR 
   		         {
   		          splitPane.setCursor(WAIT_CURSOR);
   		          oac.glb.user_sil(txtkull.getText());
   		          oac.glb.user_details_sil(txtkull.getText());
   		            grid_doldur();
   		 		    kutu_temizle();
   		 		    doldur_kutu(table_1,0);
   		 	        txtkull.setEnabled(false);
